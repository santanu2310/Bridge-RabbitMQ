...
const messages = computed(() => {
  const rawMessages =
    userStore.conversations[userStore.currentConversation?.convId as string]
      ?.messages || [];
...
<template>
  <div class="w-full h-full flex flex-col">
    ...
    >
      <div class="h-fit py-4 ml-0 mr-4 lg:ml-4 flex items-center" v-if="friend">
        <button
        ...
        >
          ...
          <div class="h-full aspect-square overflow-hidden rounded-full">
            <img
            ...
            <div
              ...
              :style="{ background: getUserColor(friend.fullName) }"
            >
              <span
                class="w-fit h-fit block text-white text-sm font-semibold"
                >{{
                  getInitials(friend.fullName || (friend.userName as string))
                }}</span
              >
            </div>
          </div>
        ...
        <div class="h-fit ml-3 flex flex-col">
          ...
          <span v-if="isOnline(friend.id)" class="text-xs text-color-heading">
            online
          </span>
          <span v-else class="text-xs text-color-heading">offline</span>
        </div>
      </div>
        ...
        >
          ...
          >
            <span
              ...
              >{{ formatDateDifference(msg.sendingTime as string, false) }}
            </span>
          </div>
          <Message :message="msg" :user-id="userStore.user.id as string" />
        </div>
      </div>
    </div>
    ...
    >
      ...
      <button
        class="h-8 mx-2 aspect-square bg-transparent border-none flex items-center justify-center"
        @click="
          showEmojiBoard ? (showEmojiBoard = false) : (showEmojiBoard = true)
        "
      >
        <IconSticker :size="66" />
      </button>
