...
export const useMessageStore = defineStore("message", () => {
  ...
  socket.on("message", async (msg) => {
    if (!messageService.isValidMessage(msg)) {
      console.error("Invalid message data:", msg);
      return;
    }

    const message = mapResponseToMessage(msg);
    await messageService.saveMessageToDB(message);
    ...
    if (!conversation) {
      await messageService.handleNewConversation(message);
    } else {
      await messageService.updateConversation(conversation, message, userStore);
    }

    if (message.senderId === userStore.user.id) {
      console.log("hendling temp message");
      await messageService.handleTempMessage(message, msg);
    } else {
      messageService.sendAcknowledgement(message, syncStore.sendMessage);
      addMessageInState(message);
    }
    console.log("finally: ", userStore.conversations);
  });
  ...
  async function sendMessage(message: string) {
    ...
    console.log("current conversation ", userStore.currentConversation);
    if (!userStore.currentConversation.convId) {
      ...
      userStore.currentConversation.convId = msg.temp_id;
    } else {
      // Old conversation
  ...
  function deleteMessageFromList(
    ...
    if (candidateIndex === -1) {
      candidateIndex = userStore.conversations[convId].messages.length - 1;
    }
    ...
    if (
      userStore.conversations[convId].messages[candidateIndex].id == targetedId
    ) {
