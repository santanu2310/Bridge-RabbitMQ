...
async def handle_online_status_update():
    ...
    while True:
        consumer = AIOKafkaConsumer(
            settings.KAFKA_TOPICS.online_status.value,
...
async def send_profile_media_update_confirmation():
    while True:
        consumer = AIOKafkaConsumer(
            settings.KAFKA_TOPICS.media_update.value,
            bootstrap_servers=settings.KAFKA_CONNECTION,
        )
        try:
            ...
            async for msg in consumer:
                # Decoding the received data
                data = json.loads(msg.value.decode("utf-8"))
                print(f"from media update {data=}")
                message = ProfileMediaUpdate.model_validate(data)

                await send_sync_message(
                    user_ids=[message.user_id], message_data=message
                )
            ...
            await consumer.stop()
            print("send_profile_media_update_confirmation stoped")
            print("Reconnecting in 5 sec")
            await asyncio.sleep(5)
